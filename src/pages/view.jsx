...
const View =()=>{
    ...
    const [feedPost, setFeedPost] = useState([{"name": "", "link": "https://www.youtube.com/watch?v=AjWfY7SnMBI"}]);
...
<img src={shuffle} className="shuffle" alt="" onClick={()=>{
...
<img src={home} onClick={()=>{navigate('/')}} className="home" alt="" />

<div className='songTile'>
<img src={grabLink(feedPost[counter]["link"], "mq")} alt="" className='songImage' />
<p className='songTitle'>{feedPost[counter]["name"]}</p>
<img src={playing?pause:play} alt="" onClick={()=>{setPlaying(!playing)}} className="controller"/>
</div>
{
  feedPost[counter]?<ReactPlayer url={feedPost[counter]["link"]} playing={playing} />:<div></div>
}
